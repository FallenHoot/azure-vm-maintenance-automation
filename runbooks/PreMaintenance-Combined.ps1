param (
    [Parameter(Mandatory=$true)][ValidateSet("PRE", "PRD")][string]$Environment,
...
# Default filter values based on environment if not specified
if (-not $NamePattern) { $NamePattern = $Environment }
if (-not $TagValue) { $TagValue = if ($Environment -eq "PRE") { "pre" } else { "prod" } }
